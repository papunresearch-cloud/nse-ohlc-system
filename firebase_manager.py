...
def init_firebase() -> None:
    ...
    try:
        if isinstance(cred_env, str) and cred_env.strip().startswith("{"):
            cred_dict = json.loads(cred_env)
            cred = credentials.Certificate(cred_dict)
        elif os.path.exists(str(cred_env)):
...
def clear_live_candle(display_name: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Firebase clear live candle failed for {display_name}: {e}")
        return False


def sanitize_key(key: str) -> str:
    """Sanitizes Firebase keys replacing invalid characters except spaces."""
    return str(key).strip().replace(".", "_").replace("$", "").replace("#", "").replace("[", "").replace("]", "").replace("/", "_")