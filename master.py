"""
MASTER ORCHESTRATOR
- Uninterrupted live market tracking (CHILD-2) and parameter engine.
- Prioritizes continuous live execution over strict historical synchronization.
- Decouples vendor sync issues (e.g. Yahoo lag/NaNs) from live Index 0 updates.
- SR Flip-Flop Power Latch (Default: ON).
- Handles pulse commands: /start, /stop, /sync.
- Embedded HTTP Server with minimal /health handling for cron-job.org keep-alive.
"""
import os
import sys
import time
...
class MasterOrchestrator:
    ...
    def run(self):
        ...
        def boot_worker():
            ...
            if all_stocks:
                logger.info(f"[STARTUP] Initial parameter run across {len(all_stocks)} stocks...")
                update_all_parameters(all_stocks)
                logger.info("[STARTUP] Initial parameters completed.")
        ...
        while _keep_running:
            try:
                ...
                # Priority 0: Watchlist vs Stocklist Heartbeat Sync
                try:
                    was_updated, _ = reconcile_stocklist_with_watchlist()
                    if was_updated:
                        logger.info("[HEARTBEAT] Change detected in watchlist. Refreshing plan...")
                        self.replan_daily_routine()
                        threading.Thread(target=self.execute_historical_sync, kwargs={"is_manual": False}, daemon=True).start()
                except Exception as e:
                    logger.error(f"[HEARTBEAT] Watchlist audit check failed: {e}")
                ...
                # Priority 2: State-Driven Date Catch-Up
                ...
                # Pre-Market Sync Window (08:00 – 08:30 IST)
                ...
                if sync_start <= now_time < sync_cutoff:
                    ...
                    if has_unsynced and (time.time() - self.last_sync_attempt_time >= SYNC_RETRY_INTERVAL_SEC):
                        logger.info("[SCHEDULE] Pre-market sync active. Retrying historical data...")
                        self.execute_historical_sync(is_manual=False)
                ...
                if status == "LIVE":
                    # 5-minute live update pass for ALL stocks (never blocked)
                    ...
                    # 15-minute parameter recalculation pass for ALL stocks
                    if (time.time() - self.last_param_calc_time) >= PARAM_UPDATE_INTERVAL_SEC:
                        all_active_scripts = list(self.script_status.keys())
                        if all_active_scripts:
                            update_all_parameters(all_active_scripts)
    ...
    def replan_daily_routine(self):
        ...
        # Reconcile and pull active stocklist
        _, stock_map = reconcile_stocklist_with_watchlist()
        if not stock_map:
            from firebase_manager import get_stocklist_mapping
    ...
    def execute_historical_sync(self, is_manual: bool = False):
        """Runs CHILD-1 historical sync. Failures log warnings but never crash the process."""
        ...
        for name, meta in list(self.script_status.items()):
            ...
            try:
                ...
                if success:
                    ...
                    # Still keep alive tracking ready
                    meta["synced"] = False
                    meta["error"] = msg
                    logger.warning(f"[{name}] Historical sync incomplete: {msg}. Live collection remains enabled.")
    ...
    def execute_live_updates(self):
        """Runs CHILD-2 for every active stock regardless of historical sync status."""
        ...
        for name, meta in self.script_status.items():
            ...
            # Never exclude: live collection runs unconditionally
            try:
    ...
    def log_detailed_sync_audit(self):
        ...
        if unsynced:
            logger.warning("[NON-BLOCKING HISTORICAL GAPS]")
            for u in unsynced:
                info = self.script_status[u]
                logger.warning(f"  ! {u:<24} | Last Attempt: {info['last_attempt_at']} | Notice: {info['error']}")
        logger.info("=" * 85)
