"""
MASTER ORCHESTRATOR
- Embedded HTTP server on port 10000 (/health, /start, /stop, /sync, /sync-screener).
- Real-time command bus listener via Firebase /system_commands/stock_event.
- Immediate targeted OHLC initialization and /param calculation on ADD.
- Admin-level targeted database purge (/stocks, /param, and /display_list/stocks) on DELETE.
- Dynamic stock discovery using get_stocklist_mapping().
- Pre-market sync window (08:00–08:30 IST) with 5-minute retry intervals.
- Live market updates (CHILD-2 via live_child.py) and parameter calculations.
"""
...
def dispatch_screener_sync_job():
    ...
    def worker():
        try:
            logger.info("[SCREENER] Starting Screener ETL Pipeline...")
            run_screener_pipeline()
            logger.info("[SCREENER] Pipeline completed successfully.")
...
class MasterOrchestrator:
    ...
    def handle_stock_addition(self, stock_name: str, ticker: str):
        """Seeds 300 historical bars, initializes live row 0, and calculates parameters."""
    ...
    def handle_stock_deletion(self, stock_name: str):
        """Purges OHLC database, parameters, and display_list entries immediately."""
        safe_key = sanitize_key(stock_name)
        ...
        # 3. Purge stock entry from display_list (supports array and map)
        try:
            display_ref = db.reference("display_list/stocks")
            current_display = display_ref.get()

            if isinstance(current_display, list):
                filtered = [s for s in current_display if sanitize_key(str(s)) != safe_key]
                display_ref.set(filtered if filtered else None)
                logger.info(f"[EVENT-HANDLER] Pruned {safe_key} from array /display_list/stocks")
            elif isinstance(current_display, dict):
                db.reference(f"display_list/stocks/{safe_key}").delete()
                if stock_name in current_display:
                    db.reference(f"display_list/stocks/{stock_name}").delete()
                logger.info(f"[EVENT-HANDLER] Pruned {safe_key} from dict /display_list/stocks")
        except Exception as e:
    ...
    def replan_daily_routine(self):
        """Forces reconciliation between /stocklist and /watchlist, preserving active entries."""
        now_ist = datetime.now(IST)
        today = now_ist.date()
        self.calendar.refresh_calendar()
    ...
    def execute_historical_sync(self, is_manual: bool = False):
        """Runs historical sync (seeds 300 bars and initializes live row)."""
    ...
    def execute_live_updates(self):
        """Runs CHILD-2 live updates using live_child.py."""
